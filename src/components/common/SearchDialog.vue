...
const filteredGroups = computed<Groups>(() => {
  ...
  for (const group of groups.value) {
    const items = group.items.filter((item) => {
      if (
        keywords.value.filter((k) => item.title.toLowerCase().indexOf(k.toLowerCase()) !== -1)
          .length !== 0
      ) {
        return true;
      }
      if (
        item.tags &&
        keywords.value.filter((k) => item.tags?.toLowerCase().indexOf(k.toLowerCase()) !== -1)
          .length !== 0
      ) {
        return true;
      }
...
function nextItem() {
  const currentIndex = filteredFlattenItems.value.findIndex(
    (item) => item.key === activeItem.value,
  );
