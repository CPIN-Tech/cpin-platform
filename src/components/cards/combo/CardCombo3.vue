    ...
    <div class="card-wrap flex flex-col">
      ...
      <n-spin :show="!loaded" class="chart grow">
        <DemoChart
          v-if="loaded"
          type="area"
          :seriesList="twoSeries ? ['Production', 'Reward'] : ['Users']"
          colorsSecondary
...
import DemoChart, {
  type DataType,
  type VueApexChartsComponent,
} from '@/components/charts/DemoApex.vue';
...
const updateTime = ref(dayjs().subtract(1, 'hour').format('DD-MM-YYYY HH:mm'));
const chartTypeValue = ref<DataType>('week');
